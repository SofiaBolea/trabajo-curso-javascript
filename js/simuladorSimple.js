...
function solicitarTurno() {
    ...
    const especialidad = prompt("Elija una especialidad (ESCRIBIR TAL CUAL APARECE EL NOMBRE EN PANTALLA):\n" + especialidades.join("\n"));
    ...
    turnos.push({ nombre, especialidad, turno });
...
function verTurnos() {
    ...
    for (let i = 0; i < turnos.length; i++) {
        const t = turnos[i];
        mensaje += `${t.turno} - ${t.nombre} (${t.especialidad})\n`;
    }
...
function cancelarTurno() {
    ...
    const confirmar = confirm(`¿Está seguro que desea cancelar el turno de ${turnos[index].nombre}?`);
    if (confirmar) {
